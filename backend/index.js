...
app.post("/login", async (req, res) => {
    ...
    if (!userInfo) return res.status(400).json({ error: true, message: "User does not exist" })
    if (userInfo.email == email && userInfo.password == password) {
        const user = { user: userInfo };
        const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "36000m", })
        return res.json({ error: false, message: "Login Successful", email, accessToken })
    } else {
        return res.status(400).json({ error: true, message: "Credentials are invalid" })
    }
});
...
app.post("/add-note", authenticateToken, async (req, res) => {
    ...
    try {
        const note = new Note({ title, content, tags: tags || [], createdOn: new Date().getTime(), updatedOn: new Date().getTime(), userId: user._id, });
        await note.save();
...
app.put("/edit-note/:noteId", authenticateToken, async (req, res) => {
    ...
    const { user } = req.user;
    if (!title && !content && !tags) return res.status(400).json({ error: true, message: "No changes provided" })
    try {
...
app.get("/search-notes/", authenticateToken, async (req, res) => {
    ...
    try {
        const matchingNotes = await Note.find({ userId: user._id, $or: [{ title: { $regex: new RegExp(query, "i") } }, { content: { $regex: new RegExp(query, "i") } },], })
